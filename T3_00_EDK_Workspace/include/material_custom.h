...
class MaterialCustom : public EDK3::Material {
 public:
  ...
  class MaterialCustomTexturedSettings : public EDK3::MaterialSettings {
    public:
        MaterialCustomTexturedSettings() {
            memset(color_, 0, sizeof(color_));
        }
        void set_color(const float v[4]) { memcpy(color_, v, sizeof(color_)); }
        const float* color() const { return color_; }
    ...
    protected:
        virtual ~MaterialCustomTexturedSettings() {}
    private:
        MaterialCustomTexturedSettings(const MaterialCustomTexturedSettings& other) {
            memcpy(color_, other.color_, sizeof(color_));
            texture_[0] = other.texture_[0];
            texture_[1] = other.texture_[1];

        }
        MaterialCustomTexturedSettings(MaterialCustomTexturedSettings&& other) {
            memcpy(color_, other.color_, sizeof(color_));
            memset(other.color_, 0, sizeof(color_));
            texture_[0] = other.texture_[0];
            texture_[1] = other.texture_[1];
        ...
        MaterialCustomTexturedSettings& operator=(const MaterialCustomTexturedSettings& other) {
            memcpy(color_, other.color_, sizeof(color_));
            texture_[0] = other.texture_[0];
            texture_[1] = other.texture_[1];
            return *this;
